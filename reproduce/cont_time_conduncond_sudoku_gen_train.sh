...
# of CondUncond on the generated sudoku dataset, then run sudoku_eval on
...
SWEEP_ROOT="outputs/sweep-conduncond"
EVAL_ONLY="${EVAL_ONLY:-0}"

for p in "${p_uncond[@]}"; do
  for T in "${timesteps[@]}"; do
    for bp in "${backprop[@]}"; do
      for disc in "${discrete_time[@]}"; do
        ...
        run_name="Cont_FLM_CondUncond_Sudoku_gen_p:${p}_T:${T}_bp:${bp}"
        ...
        if [[ "${EVAL_ONLY}" != "1" ]]; then
          python main.py \
            data=sudoku-gen \
            model=small \
            algo=cond_uncond_loop_flm \
            algo.prob_unconditional="${p}" \
