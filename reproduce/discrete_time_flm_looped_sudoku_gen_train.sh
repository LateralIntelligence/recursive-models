...
cd "${REPO_ROOT}"
gammas=(0.0) #gammas=(0.0 0.5 1.0)
timesteps=(6 12)
backprop=(6 12)
discrete_time=(True)

SWEEP_ROOT="outputs/sweep-sudoku-easy"
  ...
  for T in "${timesteps[@]}"; do
    for bp in "${backprop[@]}"; do
      for disc in "${discrete_time[@]}"; do
        ...
        run_name="Longer_Discrete_FLM_Looped_Sudoku_Easy_gen_g:${gamma}_T:${T}_bp:${bp}"
        ...
        mapfile -t ckpts < <(find "${ckpt_dir}" -maxdepth 1 -type f -name 'last.ckpt' | sort -V)
        if [[ ${#ckpts[@]} -eq 0 ]]; then
