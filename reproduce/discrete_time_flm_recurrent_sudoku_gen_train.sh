#!/usr/bin/env bash
#
# sweep_disc_flm_sudoku_gen.sh — Train a (timesteps x backprop) sweep
...
SWEEP_ROOT="outputs/recurrent-sweep"
...
for T in "${timesteps[@]}"; do
    for bp in "${backprop[@]}"; do
        for disc in "${discrete_time[@]}"; do
            ...
            # backprop_steps can't exceed num_timesteps; skip invalid combos.
            if (( bp > T )); then
                echo "skip: backprop=${bp} > timesteps=${T}"
                continue
            fi

            run_name="Discrete_FLM_Recurrent_Sudoku_gen_T:${T}_bp:${bp}"
            ...
            for ckpt in "${ckpts[@]}"; do
                ...
                # checkpoint loads; T/bp don't affect generation, so they're
